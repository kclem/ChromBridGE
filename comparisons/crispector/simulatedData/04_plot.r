library(ggplot2)
pdf('04_plot.r.pdf',width=12,height=4)

d = read.table('02_summarize_sims.py.results.txt',head=T,check.names=F,sep="\t")
p1 = ggplot(d, aes(fill=level, y=count, x=idx)) + 
           geom_bar(position="stack", stat="identity") +
	   geom_vline(xintercept=50+17, linetype='dotted') +
	   geom_segment(aes(x=50,xend=50+20,y=-1,yend=-1)) +
	   ...
	   ggtitle('CRISPECTOR analysis of simulated reads') +
